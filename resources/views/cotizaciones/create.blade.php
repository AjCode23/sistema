...
@section('name','..|Generar Cotizacion|..')

@section('ol')
 <a style="color: #e68618;" href="{{url('home')}}" class="link" title=""><i class="mdi mdi-home"></i> Home
</a>
/
 <a class="link" title=""><i class="mdi mdi-account-location"></i> Nueva Cotizacion</a>
@endsection
 ...
 return this.formatear(num);
  ...
  function agregar(id){
  	...
  	//if(confirm('¿Esta seguro de Agregar?')){
  		...
  		variables = {cliente_id:cliente_id,articulo_id:producto_id,cantidad:cantidad,pvp:pvp,descuento:descuento};
...
$('#total').html(formato.new(total.toFixed(2)));
  ...
  var cliente_id=$('#select').val();
  	if(cliente_id){
  		...
  		route="{{url('cotizacion/detalles')}}/"+cliente_id;
  		$.get(route,(dt)=>{
  			sub=0;
  			iva=0;
  			total=0;
  ...
  'question'
    ...
    Swal.fire({
  title: 'Confirme!',
  text: "Desea Finalizar la Cotización? ",
  icon: 'warning',
...
@endif
                  ...
                  @include('alerts')
                  <form id="form-guardar" action="{{url('cotizaciones')}}" method="post">
                  	{{csrf_field()}}
...
<div id="btns" style="display: none">
	...
	<button  class="btn btn-danger" onclick="guardar();" style="background: #781428; "><i class="fa fa-database"></i> Guardar Cotización</button>
