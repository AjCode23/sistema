...
@section('name','..|Permisos|..')

@section('ol')
 <a style="color: #e68618;" href="{{url('home')}}" class="link" title=""><i class="mdi mdi-home"></i> Home
</a>
/
 <a class="link" title=""><i class="mdi mdi-account-location"></i> Permisos</a>
@endsection
...
var formato= {
 ...
 return this.formatear(num);
  ...
  function del(id){
    if(confirm("¡Desea Inabilitar este Permiso?")){
      $('#tr_'+id).hide();
      $('#delete_id').val(id);
  ...
  function del(id){
   ...
   function act(id){
    if(confirm("¡Desea Activar esta Permiso?")){
      $('#status_'+id).html(`<b class="text-green" title="Categoria Activa"> <i class="fa fa-check"></i> <small>Activa</small></b> `);

      $('#btn_'+id).html(` <button onclick="del(`+id+`)" class="btn btn-outline-warning btn-xs"><i class="mdi mdi-close"></i></button> `);
              ...
              <div class="card-header">
                ...
                <div class="card-tools">
                  <ul class="nav nav-pills ml-auto">
                    <li class="nav-item">
                      <a class="nav-link btn-success active" href="{{url('permisos/create')}}" style="color: #fff !important;" ><i class="mdi mdi-plus-circle"></i> Nueva Permiso</a>
                    </li>
                      &nbsp;
                       <li class="nav-item">
                      <a class="nav-link active" href="{{url('permisos/pdf')}}"  style="color: #fff !important;"><i class="mdi mdi-file-pdf"></i> Reporte</a>
                    </li>
                 ...
                 <tr>
                  ...
                  <tr id="tr_{{$c->id}}">
                    <td width="30px;" class="text-center">
                      ...
                      <a href="{{url('permisos',$c->id)}}/edit" title="Editar Cotización" class="btn btn-outline-info btn-xs"><i class="mdi mdi-pencil"></i></a>

                      <a href="{{url('permisos',$c->id)}}/destroy" title="Eliminar Permiso" class="btn btn-outline-danger btn-xs"><i class="mdi mdi-delete-empty"></i></a>
		                  <span id="btn_{{$c->id}}">
		                  	
		                     @if($c->status == 1)
		                      <button onclick="del({{$c->id}})" class="btn btn-outline-warning btn-xs"><i class="mdi mdi-close"></i></button>
